...
def test_the_card_names_what_ran(performed):
    ...
    assert "One-Way ANOVA" not in text, (
...
def test_the_planned_model_is_used_when_nothing_ran():
    ...
    assert "Model: One-Way ANOVA" in _design("one_way_anova", {"blocked": True})
    assert "Model: One-Way ANOVA" in _design("one_way_anova", {})
...
def test_a_sentinel_is_not_a_model_name(sentinel):
    ...
    assert "Model: One-Way ANOVA" in text, text
