...
""",
    },
    {
        ...
        "html": (
            ...
            "<p>Samples are dependent when the values in one group are tied to the values in another:</p>"
            "<ul>"
            "<li>The <b>same subject</b> is measured at different time points</li>"
            "<li>Measurements are naturally paired, such as the left and right eye of one person</li>"
            "<li>Subjects are matched into pairs before the measurement</li>"
            "</ul>"
            "<h3>How to arrange the data</h3>"
            "<p>The app pairs values by row position, so each group needs the <b>same number</b> of measurements in a <b>matching order</b>. Row 1 in group A and row 1 in group B must be the same subject, row 2 the next subject, and so on. Unequal group sizes are rejected; a mismatched row order is not detected, so it would silently pair the wrong values.</p>"
            "<h3>Which test the app runs</h3>"
            "<ul>"
            "<li><b>Two groups:</b> a paired t-test, or the Wilcoxon signed-rank test when the paired differences are not normal</li>"
