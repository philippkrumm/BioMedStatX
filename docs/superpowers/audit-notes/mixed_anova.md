...
| 6 (rewritten) | Common mistake: time points as separate columns fails; the app does not auto-reshape a wide file that has a group column | correct (was under-explained before) | `_detect_wide_format` returns `None` when any categorical (non-subject, non-numeric) column has exactly 2 unique values, i.e. a group column: `for col in categorical_cols: if df[col].nunique() == 2: return None` (`statistical_analyzer_autopilot_ui.py:161`-`165`). The recipe's wide example has a 2-level Group column ("Control"/"Treatment"), so it is not melted. Even the melt path drops non-value columns: `_pivot_wide_to_long` keeps only `[subject_col] + value_cols` (`statistical_analyzer_autopilot_ui.py:176`-`188`), so a group column would be lost anyway. Auto-melt runs on every load/sheet-switch via `_ap_maybe_pivot` (`pipeline:1002`, `:1043`) but no-ops here. |
...
`statistical_analyzer_autopilot_ui.py:161`-`165`) and would lose the group column even if it were
